...
<body>
	...
	<!-- 
		아래는 if(number % 2 == 0) 와 동일한 조건의 if문
		해당 조건의 결과는 result에 저장된다.
	-->
	...
	<!-- 
		JSTL의 if 태그는 else 구문이 따로 없으므로 두개의 if 태그를
		통해 else와 같이 사용해야 한다.
		만약 여러개의 조건을 사용할 경우라면 choose태그를 사용하는게 더 좋다.
	-->
	...
	<c:if test="${ not empty param.user }">
		...
		<br />
		<!--  
		아이디, 패스워드의 일치 여부를 판단한 후 result에 결괄르 저장한다.
	...
	<c:if test="${not result and !(param.user eq null and param.pass eq null)}">
		아이디와 비번을 확인하세요 <br />
	</c:if> --%>
