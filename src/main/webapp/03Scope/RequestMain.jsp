...
/*
request 영역
	: 클라이언트가 요청을 할때마다 새로은 request 객체가 생성되어
	같은 요청을 처리하는데 사요되는 모든 JSP페이지가 영역을 공유한다.
	즉 request영역에 저장된 정보는 현재 페이지와 포워드(foward)된 
	페이지까지 공유된다.
*/
...
<body>
	...
	<%
		...
		request.removeAttribute("requestInteger");//존재하지 않는 정보이지만 에러가 발생하지는 않음.
	%>
