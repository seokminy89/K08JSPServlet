...
/*
add 계열의 메서드를 통해 응답헤더를 추가한다.
addDateHeader(문자열, long타입의 타임스탬프)
	: 응답헤더중 날짜를 추가하는 메소드로, 세계 표준시를 사용해서 설정한다.
	대한민국은 표준시 +09, 즉 9시간이 느리므로 09시로 설정해야 정상적인
	날짜가 출력된다. 만약 09시를 표시 하지 않거나 이전의 시간이 설정되면
	하루 전 즉 어제의 날짜가 출력되어 버린다.
*/
response.addDateHeader("myBirthday", add_date);//날짜 형식의 응답헤더를 추가
...
사이트에 접속할 때마다 F5를 누른것처럼 새로운 정보를 서버로부터 
받아 갱신하게 된다.
*/
...
<body>
	...
	*/
	...
	동일하게 사용할 때 헤더값이 수정되는게 아니라 추가된다.
	*/
